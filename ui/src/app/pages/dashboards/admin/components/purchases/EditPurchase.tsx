...
export const EditPurchase = ({ open, onOpenChange, purchase, products, updatePurchase }: EditPurchaseProps) => {
  ...
  useEffect(() => {
    if (purchase) {
      setFormData({
        product_id: String(purchase.product_id ?? purchase.product?.id ?? ''),
        quantity: String(purchase.quantity ?? ''),
        unit_cost: String(purchase.unit_cost ?? purchase.price ?? ''),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-106.25'>
        <DialogHeader>
          ...
          <DialogDescription>Update the purchase record.</DialogDescription>
        </DialogHeader>
