...
export const AddStockTransfer = ({ createTransfer, branches, products }: any) => {
  ...
  const [form, setForm] = useState({ from_branch_id: '', to_branch_id: '', notes: '', items: [{ business_branch_product_id: '', quantity_expected: '' }] });

  const addItem = () => setForm((p) => ({ ...p, items: [...p.items, { business_branch_product_id: '', quantity_expected: '' }] }));
  const removeItem = (i: number) => setForm((p) => ({ ...p, items: p.items.filter((_, idx) => idx !== i) }));
  const updateItem = (i: number, field: string, value: string) => setForm((p) => ({ ...p, items: p.items.map((item, idx) => idx === i ? { ...item, [field]: value } : item) }));

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const validItems = form.items.filter((i) => i.business_branch_product_id && i.quantity_expected);
    if (!form.from_branch_id || !form.to_branch_id) { toast.error('Select source and destination branches'); return; }
    ...
    try {
      await createTransfer({ ...form, items: validItems.map((i) => ({ ...i, quantity_expected: Number(i.quantity_expected) })) }).unwrap();
      toast.success('Stock transfer created');
      setOpen(false);
      setForm({ from_branch_id: '', to_branch_id: '', notes: '', items: [{ business_branch_product_id: '', quantity_expected: '' }] });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='sm:max-w-2xl max-h-[85vh] overflow-y-auto'>
        <DialogHeader>
          ...
          <DialogDescription>Move stock between branches. Items will be deducted from source on dispatch.</DialogDescription>
        </DialogHeader>
            ...
            {form.items.map((item, idx) => (
              <div key={idx} className='flex gap-3 items-end p-3 bg-muted/50 rounded-lg'>
                <div className='flex-1 space-y-1'>
                  ...
                  <Select value={item.business_branch_product_id} onValueChange={(v) => updateItem(idx, 'business_branch_product_id', v)}>
                ...
                <div className='w-24 space-y-1'>
                  ...
                  <Input type='number' value={item.quantity_expected} onChange={(e) => updateItem(idx, 'quantity_expected', e.target.value)} />
                </div>
