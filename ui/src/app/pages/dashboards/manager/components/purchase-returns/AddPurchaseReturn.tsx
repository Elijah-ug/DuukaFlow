...
export const AddPurchaseReturn = ({ addPurchaseReturn }: AddPurchaseReturnProps) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return availableItems.filter(
      (item) =>
        ...
        String(item.purchase_id).includes(q),
    );
  ...
  const toggleItem = (item: (typeof availableItems)[0]) => {
    setSelectedItems((prev) => {
      ...
      if (exists) {
        return prev.filter((s) => s.purchase_item_id !== item.purchase_item_id);
      }
      return [
        ...prev,
        {
    ...
    >
      ...
      <DialogContent className='sm:max-w-156.25 max-h-[85vh] overflow-y-auto'>
        <DialogHeader>
          ...
          <DialogDescription>Return items to supplier from any purchase.</DialogDescription>
        </DialogHeader>
