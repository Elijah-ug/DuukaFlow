...
export const AddSaleReturn = ({ addSaleReturn }: AddSaleReturnProps) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return availableItems.filter(
      (item) =>
        ...
        String(item.sale_id).includes(q),
    );
  ...
  const toggleItem = (item: (typeof availableItems)[0]) => {
    setSelectedItems((prev) => {
      ...
      if (exists) {
        return prev.filter((s) => s.sale_item_id !== item.sale_item_id);
      }
      return [
        ...prev,
        {
  ...
  const updateQty = (saleItemId: number, qty: number) => {
    setSelectedItems((prev) =>
      prev.map((s) => (s.sale_item_id === saleItemId ? { ...s, qty_to_return: qty } : s)),
    );
  };
    ...
    >
      ...
      <DialogContent className='sm:max-w-156.25 max-h-[85vh] overflow-y-auto'>
        <DialogHeader>
          ...
          <DialogDescription>
            Select items from completed sales. A single return can span multiple sales.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className='grid gap-4 py-4'>
            ...
            <div className='grid grid-cols-4 items-center gap-4'>
              ...
              <Input
                ...
                placeholder='e.g. Customer not satisfied'
              />
            </div>
