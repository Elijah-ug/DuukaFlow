...
|          |                                                                                                                                                                                                            |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **What** | Storage locations (warehouses, shelves, bins) within a branch. Each `product` can be assigned to one or more locations with quantity per location. Supports picking and stock counting.                    |
...
| **How**  | Create `StockLocation` model (branch_id, name, type: warehouse/shelf/bin). Add `location_id` to `product` or create a pivot `inventory_locations` table. Update stock movements to track location changes. |
...
|          |                                                                                                                                                                                                                     |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **What** | Track changes to cost price and selling price over time for each `product`. Includes who changed it, when, and the previous/next values.                                                                            |
